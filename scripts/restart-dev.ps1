...
$vitePids = netstat -ano 2>$null | Select-String ":5173\s" | ForEach-Object {
    ($_ -split '\s+')[-1]
} | Sort-Object -Unique
...
Write-Host ""
Write-Host "[3/4] Starting infrastructure (PostgreSQL, RabbitMQ, Redis, Mailpit, pgAdmin)..." -ForegroundColor Yellow
...
if (-not $InfraOnly) {
    ...
    if (Test-Path (Join-Path $frontendDir "package.json")) {
        ...
        $viteCheck = netstat -ano 2>$null | Select-String ":5173\s"
        if ($viteCheck) {
