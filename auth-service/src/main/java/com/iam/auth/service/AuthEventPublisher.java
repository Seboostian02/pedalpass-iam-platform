...
public class AuthEventPublisher {
    ...
    public void publishAccountLocked(UUID userId, String email, String ipAddress, int lockoutMinutes) {
        AuditEvent event = AuditEvent.builder()
                .eventId(UUID.randomUUID())
                .userId(userId)
                .userEmail(email)
                ...
                .metadata(Map.of(
                        ...
                        "lockoutMinutes", String.valueOf(lockoutMinutes)
                ))
                .build();

        rabbitTemplate.convertAndSend(
                RabbitMQConstants.EVENTS_EXCHANGE,
                RabbitMQConstants.AUTH_ACCOUNT_LOCKED,
                event);
        log.warn("Published account locked event for {}", email);
    }

    public void publishPasswordResetRequested(String email, String resetLink) {
        AuditEvent event = AuditEvent.builder()
                .eventId(UUID.randomUUID())
                .userEmail(email)
        ...
        rabbitTemplate.convertAndSend(
                RabbitMQConstants.EVENTS_EXCHANGE,
                RabbitMQConstants.AUTH_PASSWORD_RESET,
                event);
        log.debug("Published password reset requested event for {}", email);
    }
