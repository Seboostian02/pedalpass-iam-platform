...
import com.iam.auth.model.PasswordResetToken;
import com.iam.auth.model.RefreshToken;
import com.iam.auth.model.UserCredential;
import com.iam.auth.repository.PasswordResetTokenRepository;
...
public class AuthService {
    ...
    @Value("${auth.password-reset-token-expiration-hours:1}")
    ...
    public void requestPasswordReset(String email) {
        userCredentialRepository.findByEmail(email).ifPresent(credential -> {
            ...
            eventPublisher.publishPasswordResetRequested(email, resetLink);

            log.info("Password reset requested for: {}", email);
        });
        // Silently ignore if email not found (security: no email enumeration)
    }

    public void validateResetToken(String token) {
        PasswordResetToken resetToken = passwordResetTokenRepository.findByTokenAndUsedFalse(token)
                .orElseThrow(() -> new BaseException("Invalid or expired reset link", HttpStatus.BAD_REQUEST));

        if (resetToken.getExpiresAt().isBefore(LocalDateTime.now())) {
            throw new BaseException("Invalid or expired reset link", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        UserCredential credential = userCredentialRepository.findById(resetToken.getUserId())
                .orElseThrow(() -> new BaseException("User not found", HttpStatus.NOT_FOUND));
        ...
        eventPublisher.publishPasswordChanged(credential.getId(), credential.getEmail());

        log.info("Password reset completed for: {}", credential.getEmail());
    }
    ...
    private void handleFailedLogin(UserCredential credential, String ipAddress) {
        ...
        if (credential.getFailedAttempts() >= maxFailedAttempts) {
            ...
            eventPublisher.publishAccountLocked(
                    credential.getId(), credential.getEmail(), ipAddress, lockoutDurationMinutes);
        }
        userCredentialRepository.save(credential);
