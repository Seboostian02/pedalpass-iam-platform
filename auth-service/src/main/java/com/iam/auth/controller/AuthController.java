...
public class AuthController {
    ...
    @Operation(summary = "Request password reset", description = "Sends a password reset link to the email if it exists. Always returns success for security.")
    @ApiResponses({
    ...
    public ResponseEntity<ApiResponse<Void>> validateResetToken(@RequestParam String token) {
        ...
        return ResponseEntity.ok(ApiResponse.<Void>success("Token is valid", null));
    }

    @PostMapping("/password/reset")
    @Operation(summary = "Reset password", description = "Resets user password using a valid reset token. Token becomes invalid after use.")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Password reset successfully"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Invalid/expired token or weak password")
    })
