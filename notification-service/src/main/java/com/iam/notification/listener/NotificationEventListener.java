...
public class NotificationEventListener {
    ...
    @RabbitListener(queues = RabbitMQConstants.NOTIFICATION_ACCOUNT_LOCKED_QUEUE)
    public void handleAccountLockedEvent(AuditEvent event) {
        log.warn("Received account locked event for {}", event.getUserEmail());
        String lockoutMinutes = String.valueOf(event.getMetadata().getOrDefault("lockoutMinutes", "30"));
        String ipAddress = event.getIpAddress() != null ? event.getIpAddress() : "unknown";

        String emailBody = String.format(
                "Hello,\n\n" +
                "Your IAM Platform account has been locked for %s minutes due to 5 consecutive failed login attempts.\n\n" +
                "IP address: %s\n\n" +
                "If this was not you, please contact support immediately after your account is unlocked.\n\n" +
                "Best regards,\nIAM Platform Team",
                lockoutMinutes, ipAddress);
    ...
    public void handlePasswordResetEvent(AuditEvent event) {
        log.info("Received password reset event for {}", event.getUserEmail());
        String resetLink = String.valueOf(event.getMetadata().get("resetLink"));

        String emailBody = String.format(
                "Hello,\n\n" +
                "We received a request to reset your IAM Platform password.\n\n" +
        ...
        notificationService.sendEmailDirect(
                event.getUserEmail(),
