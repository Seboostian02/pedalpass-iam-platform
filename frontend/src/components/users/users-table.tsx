    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={roleFilter} onValueChange={setRoleFilter}>
          ...
          <SelectContent>
            ...
            {(roles ?? []).map((role) => (
              <SelectItem key={role.name} value={role.name}>{tEnum('role', role.name)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={search || roleFilter !== 'all' ? t('users.adjustFilters') : t('users.noUsersYet')}
        />
      ) : (
