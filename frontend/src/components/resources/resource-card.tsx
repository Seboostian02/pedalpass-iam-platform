      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <Badge variant="outline" className={`text-xs ${CATEGORY_COLORS[resource.resourceCategory] || ''}`}>
            {tEnum('resourceCategory', resource.resourceCategory)}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col space-y-3">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
          ...
          {resource.capacity && (
            <span className="flex items-center gap-1">
              ...
              {t('resources.cap', { value: resource.capacity })}
            </span>
          )}
          <span className="flex items-center gap-1">
            {resource.requiresApproval
              ? <><Lock className="h-3 w-3" /> {t('resources.requiresApproval')}</>
              : <><LockOpen className="h-3 w-3" /> {t('resources.autoApproved')}</>}
          </span>
        </div>
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-1">
          <Badge variant="outline" className={resource.active
            ...
            {resource.active ? t('common.active') : t('common.inactive')}
          </Badge>
