...
export function useResolveAlert() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(i18n.t('toast.alertResolved'));
    },
    onError: (error: Error) => {
      toast.error(error.message || i18n.t('toast.failedResolveAlert'));
    },
  });
}
...
export function useDismissAlert() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['alerts'] });
      toast.success(i18n.t('toast.alertDismissed'));
    },
    onError: (error: Error) => {
      toast.error(error.message || i18n.t('toast.failedDismissAlert'));
    },
  });
}
