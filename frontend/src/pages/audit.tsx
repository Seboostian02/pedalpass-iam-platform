      ...
      <Tabs defaultValue="logs">
        <TabsList className="w-full sm:w-auto">
          <TabsTrigger value="logs" className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">{t('audit.auditLabel')}</span> {t('audit.auditLogs')}
          </TabsTrigger>
          <TabsTrigger value="alerts" className="flex items-center gap-2">
            <ShieldAlert className="h-4 w-4" />
            <span className="hidden sm:inline">{t('audit.securityLabel')}</span> {t('audit.securityAlerts')}
        ...
        <TabsContent value="logs" className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={severityFilter} onValueChange={setSeverityFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">{t('audit.allSeverity')}</SelectItem>
          ...
          {logsQuery.isLoading ? (
            ...
            <EmptyState
              ...
              title={t('audit.noLogsFound')}
              description={logSearch || severityFilter !== 'all' ? t('audit.adjustFilters') : t('audit.noLogsYet')}
            />
          ) : (
            <>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{t('audit.resourceColumn')}</TableHead>
        ...
        <TabsContent value="alerts" className="space-y-4">
          {alertsQuery.isLoading ? (
            ...
            <EmptyState
              ...
              title={t('audit.noAlerts')}
              description={t('audit.systemSmooth')}
            />
          ) : (
            <>
              <div className="overflow-x-auto rounded-md border border-border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t('audit.createdColumn')}</TableHead>
                      <TableHead>{t('audit.typeColumn')}</TableHead>
                      <TableHead>{t('audit.severityColumn')}</TableHead>
                      <TableHead>{t('audit.statusColumn')}</TableHead>
                      <TableHead>{t('audit.userColumn')}</TableHead>
                      <TableHead>{t('audit.descriptionColumn')}</TableHead>
                      <TableHead className="text-right">{t('audit.actionsColumn')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
