  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={t('accessRequests.description')}
      />
      ...
      <Tabs defaultValue="my-requests">
        <TabsList className="w-full sm:w-auto">
          <TabsTrigger value="my-requests" className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">{t('accessRequests.myLabel')}</span> {t('accessRequests.myRequests')}
          ...
          {canReview && (
            <TabsTrigger value="pending-review" className="flex items-center gap-2">
              ...
              {t('accessRequests.pendingReview')}
        ...
        <TabsContent value="my-requests" className="space-y-4">
          {myRequestsQuery.isLoading ? (
            ...
            <EmptyState
              ...
              title={t('accessRequests.noRequestsYet')}
            ...
            <>
              <div className="overflow-x-auto rounded-md border border-border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t('accessRequests.resourceColumn')}</TableHead>
                      <TableHead>{t('accessRequests.accessLevelColumn')}</TableHead>
                      <TableHead>{t('accessRequests.statusColumn')}</TableHead>
                      <TableHead>{t('accessRequests.scheduleColumn')}</TableHead>
                      <TableHead>{t('accessRequests.submittedColumn')}</TableHead>
                      <TableHead>{t('accessRequests.reviewCommentColumn')}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                          ...
                          {req.scheduledStart && req.scheduledEnd ? (
                            <div className="flex items-center gap-1.5">
                              ...
                              <div>
                                ...
                                <div className="text-xs opacity-70">{t('common.to')} {format(new Date(req.scheduledEnd), 'MMM dd, HH:mm')}</div>
                              </div>
                            </div>
                          ) : (
          ...
          <TabsContent value="pending-review" className="space-y-4">
            {pendingQuery.isLoading ? (
              ...
              <EmptyState
                ...
                title={t('accessRequests.allCaughtUp')}
                description={t('accessRequests.noPendingDescription')}
              />
            ) : (
              <>
                <div className="overflow-x-auto rounded-md border border-border">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{t('accessRequests.userColumn')}</TableHead>
                        <TableHead>{t('accessRequests.resourceColumn')}</TableHead>
                        <TableHead>{t('accessRequests.accessLevelColumn')}</TableHead>
                        <TableHead>{t('accessRequests.scheduleColumn')}</TableHead>
                        <TableHead>{t('accessRequests.justificationColumn')}</TableHead>
                        <TableHead>{t('accessRequests.submittedColumn')}</TableHead>
                        <TableHead className="text-right">{t('accessRequests.actionColumn')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
